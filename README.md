...
Spell hero-mesh prompts live in `tools/spell-vfx-prompts.json`. Export the optional meshes into `public/models/` with the local generator, run `npm run assets:optimize --` before publishing, and list enabled families in `public/models/spell-vfx-assets.json` (`fire`, `fireball`, `frost`, `light`, `chrono`). The browser automatically falls back to the pooled procedural spell layers when a generated mesh is not present.

The current hero asset recipes are ready for a local Blender install:

```text
blender --background --python scripts/generate_player_heroes.py
blender --background --python scripts/generate_fp_wand_hero.py
...
The shipped Steam Blender recipe uses packed, deterministic albedo maps,
authored layered geometry, and Draco-compressed GLBs. On this machine it is
located at `C:\\Program Files (x86)\\Steam\\steamapps\\common\\Blender\\blender.exe`;
set `BLENDER_BIN` to that executable (or your own Blender install) instead of
hard-coding the path in scripts. Each recipe registers its output in
`public/models/hero-assets.json` or `public/models/spell-vfx-assets.json`, after
which the browser loads it ahead of the existing sorcerer/knight and procedural
wand fallbacks. The shared `src/shared/worldCollision.js` module
is used by both the browser and server so projectile sweeps, ground fields, and
impact bursts stop at the first floor, ceiling, wall, or registered blocker;
server-side validation remains authoritative for damage and through-wall checks.
